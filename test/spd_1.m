...
filebase = 'C:\testdata\spd_1_source';
...
[systemDelay(2), systemDelay(1)] = finddelays( Parameters.track,Parameters.transmitGeometry,Parameters.C,Parameters.timeSeriesParameters(2) );
%[P, blockTimes] =processradarblock( filebase,pulse_replica,systemDelay);

ranges = ([1:1:Parameters.blockSize]-1) * C/Fs * .5+systemDelay(1)* C * .5;
...
xi = [-100:1:100] + trackCenter(1);
yi = [-100:1:100] +  trackCenter(2);
xi = [0] + trackCenter(1);
yi = [-30:.05:30] +  trackCenter(2);
% xi = [0 50] + trackCenter(1);
% yi = [0 0] +  trackCenter(2);
zi = [0 ] +  trackCenter(3);
[X,Y,Z] = meshgrid(xi,yi,zi);
%%
[Px] =bpm( filebase,prf,pulse_replica,ranges,track,geometry,X,Y,Z);
%%
figure
...
%%
az = atan2(yi,trackCenter(1)) * 180/pi;
az(az<0) = az(az<0)+360;
%%
[power az de parameters] = beampattern(Fc,az,zeros(size(az)),[],[], [],[],[180 0],C);
