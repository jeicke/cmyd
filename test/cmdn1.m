...
close all
 ...
 axis equal
%%
BW = Parameters.timeSeriesParameters(1);
T = Parameters.timeSeriesParameters(2);
pulse_replica=m_chirp(BW,T,0,2*BW);

SLL = 20*log10(T*BW);

N = 8*2^nextpow2(length(pulse_replica));
s = zeros(N,1);
mf = pulse_replica.*chebwin(length(pulse_replica),SLL);
s(1:length(pulse_replica))= mf;%pulse_replica;
s = circshift(s,N/2-length(pulse_replica)/2);
m = fft(s,N).';

n = fft(mf(end:-1:1)'.',N);
hold on;
%plot(20*log10(abs(ifft(m.*n.'))))

[result] = pulsecompress(mf(end:-1:1)'.', s,length(mf)*8,length(mf));
plot(20*log10(abs(result)))
%%
...
[Px] =bpm( filebase,prf,pulse_replica,ranges,track,[0 0 0],xi,yi,zi,...
    'windowrange',@chebwin,'windowcrossrange',@chebwin);
