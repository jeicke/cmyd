...
sigma(2) = 1;
sigma(3) = 1;
scatteredpoint(1,2) = 0;
scatteredpoint(2,2) = 20;
...
%%
xi = [0] + trackCenter(1);
yi = [-100:.5:100] +  trackCenter(2);
zi = [0 ] +  trackCenter(3);
[Px] =bpm( filebase,prf,pulse_replica,ranges,track,[0 0 0],xi,yi,zi,...
    'windowrange',@hann,'windowcrossrange',@hann,'cuda',false,'timewindow',1,'timeoverlap',.25);
[Pxu] =bpm( filebase,prf,pulse_replica,ranges,track,[0 0 0],xi,yi,zi,'cuda',false);
[Pxr] =bpm( filebase,prf,pulse_replica,ranges,track,[0 0 0],xi,yi,zi,...
    'windowrange',@hann,'windowcrossrange',@hann,'cuda',false);
%%
[Pmt] =bpm( filebase,prf,pulse_replica,ranges,track,[0 0 0],xi,yi,zi,'cuda',false,'multitaper',true);
%%
plot(yi,20*log10(abs(Px)),'g')
hold on;
grid on
plot(yi,20*log10(abs(Pxr)),'r')
plot(yi,20*log10(abs(Pxu)),'b')
plot(yi,20*log10(abs(Pmt)),'k')
