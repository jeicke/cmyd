function [timeseries, extra, x] = delayfilterblock(transmittedTimeseries, state, clutterVoltage,...
    L,  track ,Fs, Fc,C,...
    tStart,tEnd,antPos,parameters,first,rangeCorrect,sensorResponse,transmitGeometry,transmitOrientation,transmitShading,beamPower,Tz)
...
state.create = false;
x = state.chirp;%transmittedTimeseries(timess-tStart,state,parameters,Fs);
x(abs(x)==0) = [];
x = (x(:));
