...
if(~isempty(transmitGeometry))
    ...
    for ii = 1:length(azt)
        ...
        if(~iscell(transmitGeometry))
            ...
            [powerh] = beampattern(Fc,180/pi * aztransmit(:,ii),...
                180/pi*detransmit(:,ii),ones(size(geometry,1),1),[], transmitShading{1},geometry,[],C);
    ...
    beamAttenuation = ones(size(ranges));
end
 beamAttenuation = cast( single(beamAttenuation));
clutterVoltage = cast(single(clutterVoltage));
rangeAttenuationTransmitterScatter = cast(single(rangeAttenuationTransmitterScatter));
rangeAttenuationScatterReceiver = cast(single(rangeAttenuationScatterReceiver));
