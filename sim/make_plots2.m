...
transmitters = 1;
mimo;
p_mimo = 20 * log10(abs(squeeze(Dbf_sweep_mimo(151,15,:))));
p_simo = 20 * log10(abs(squeeze(Dbf_sweep_simo(151,15,:))));
...
transmitters = 2;
mimo;
s = 20 * log10(abs(squeeze(Dbf_sweep_el_mimo(342,1,:))));
...
plot(f_simo,10 * log10(sinr_stap_simo),'b',f_mimo,10 * log10(sinr_stap_mimo)-10*log10(transmitters),'r','linewidth',2);
grid
xlabel('FREQUENCY (HZ)')
ylabel('SINR (dB)')
legend('SIMO','MIMO');
