...
for ii = 1:3
    if(~uav )
        [doppler_dwell_time power_dwell_time] = gmtiscript(freqs(ii),snr(ii),cpi(ii));
    else
...
if(~uav )
    legend('5.5 GHz, 13.5 dB SNR, 3 CPI','10 GHz, 11.5 dB SNR, 5 CPI','17 GHz, 10.5 dB SNR, 7 CPI','STAR LEVEL 2 BOUND','POWER BOUND','DOPPLER BOUND','location','northwest')
else
...
if(~uav )
    label = sprintf('C:/figures/time_single_beam_isr');
else
    label = sprintf('C:/figures/time_single_beam_uav');
end
