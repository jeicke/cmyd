...
function [sar_dwell_time_beam  power_dwell_time az_beams power_dwell_time_band doppler_dwell_time_band X Y] = ...
                                                                  sarscript(operating_frequency,SNR,ncpi,Ptransmit,...
                                                                  radar_height,minmaxrange,width,height,NF,losses,...
                                                                  T,rcs,azimuth_search,spoil_parameter,...
                                                                  range_resolution,sim_recv_beams,elev_beam_ranges,spoil,...
                                                                  compute_bandgraph,velocity)
% set parameter
...
if(~exist('range_resolution','var')||isempty( range_resolution))
    range_resolution = 1; %  in m/s
end
...
sar_dwell_time_beam = wavelength*(beam_max_range_transmit * 1000)/(2*velocity*range_resolution).*1./abs(cosd(beam_az_transmit));
