...
for ii = 1:3
    ...
    if(~uav )
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(freqs(ii),snr(ii),cpi(ii),[],[],[],[],[],[],[],[],[],[],[],[],1);
        ratio = ceil(doppler_dwell_time./power_dwell_time);
        ratio(find(ratio >32)) = 32;
        spoil = ratio(floor(length(az_beams)/2):length(az_beams):end);
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(freqs(ii),snr(ii),cpi(ii),[],[],[],[],[],[],[],[],[],[],[],[],1,[],spoil);
    else
...
for ii = 1:3
    if(~uav )
        ...
        [doppler_dwell_time power_dwell_time] = gmtiscript(freqs(ii),snr(ii),cpi(ii),150,5,[7 100],.5,.2,[],[],[],[],[],[],[],4);
    end
...
if(~uav )
    legend('5.5 GHz, 13.5 dB SNR, 3 CPI','10 GHz, 11.5 dB SNR, 5 CPI','17 GHz, 10.5 dB SNR, 7 CPI','STAR LEVEL 2 BOUND','POWER BOUND','DOPPLER BOUND','location','northeast')
else
...
if(~uav )
    label = sprintf('C:/figures/time_multi_beam_beamoptimized_isr');
else
    label = sprintf('C:/figures/time_multi_beam_beamoptimized_uav');
end
