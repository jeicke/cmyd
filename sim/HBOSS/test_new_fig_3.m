...
for ii  = 1:4
    ...
    for jj = 1:length(f)
        ...
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(f(jj),snr(jj),cpi(jj),power(ii),altitude(ii),range(ii,:),width(ii),height(ii));
        ...
        [doppler_dwell_time power_dwell_time] = gmtiscript(f(jj),snr(jj),cpi(jj),power(ii),altitude(ii),...
                                                           range(ii,:),width(ii),height(ii),[],[],[],[],[],[],[],[],[],ratio);
        ...
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(f(jj),snr(jj),cpi(jj),power(ii),altitude(ii),...
                                                           range(ii,:),width(ii),height(ii),[],[],[],[],[],[],[],[],[],spoil);
        ppt_3(ii,jj) = sum(power_dwell_time);
        ddt_3(ii,jj) = sum(doppler_dwell_time);
        
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(f(jj),snr(jj),cpi(jj),power(ii),altitude(ii),...
                                                           range(ii,:),width(ii),height(ii),[],[],[],[],[],[],[],[],elev_ranges{jj},[]);
        ratio = ceil(doppler_dwell_time./power_dwell_time);
        ratio(find(ratio >beams(ii))) = beams(ii);
        spoil = ratio(floor(length(az_beams)/2):length(az_beams):end);
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(f(jj),snr(jj),cpi(jj),power(ii),altitude(ii),...
                                                           range(ii,:),width(ii),height(ii),[],[],[],[],[],[],[],[],elev_ranges{jj},spoil);
        ppt_4(ii,jj) = sum(power_dwell_time);
        ddt_4(ii,jj) = sum(doppler_dwell_time);
                                                       
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(f(jj),16.5,1,power(ii),altitude(ii),...
                                                           range(ii,:),width(ii),height(ii),[],[],[],[],[],[],[],[],elev_ranges{jj},[]);
        ratio = ceil(doppler_dwell_time./power_dwell_time);
        ratio(find(ratio >beams(ii))) = beams(ii);
        spoil = ratio(floor(length(az_beams)/2):length(az_beams):end);
