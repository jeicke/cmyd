...
%What do you mean by "explicitly putting in ./ and .* operations"?  I've been trying to figure out how that can be done un-explicitly and have failed.  Remember, * and / are very different operations and can't be substituted for .* and ./ even if it looks like it works.
...
for ii = 1:3
    if(~uav )
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(freqs(ii),snr(ii),cpi(ii),[],[],[],[],[],[],[],[],[],[],[],[],1,elev_ranges{ii});
        ...
        [doppler_dwell_time power_dwell_time az_beams] = gmtiscript(freqs(ii),snr(ii),cpi(ii),[],[],[],[],[],[],[],[],[],[],[],[],1,elev_ranges{ii},spoil);
...
if(~uav )
    label = sprintf('C:/figures/time_spoiled_elev_beam_beamoptimized_isr');
else
