function [clutter_doppler, EL] = create_doppler(AZ,velocity,range,frequency,altitude)
...
for ii = 1:length(D)
    clutter_doppler(:,ii) =  2 * velocity' * [A.*C(ii);B.*C(ii);ones(size(A)) * D(ii)] * frequency/299792458;
end
