...
for ii = angle
    ...
    sinr_all(:,count) = 10*log10(sinr_all(:,count));
    if PLOT_SPEED
        plot(vc*f,sinr_all(:,count),colors(count),'linewidth',3)
       
        hold on;
    else
        plot(f,sinr_all(:,count),colors(count),'linewidth',3)
    end
    count = count + 1;
end
...
for ii =angle
    if(~ISBISTATIC)
        ...
        s = mod(v/(c/operating_frequency) * cosd(ii) * cos(EL(range_gate)),prf);
    end
    if(s>prf/2)
        s = s - prf;
    end
    ...
    if(~ISBISTATIC)
        ...
        df1 =  v/(c/operating_frequency) * cos(EL(range_gate)) * 2 * (cosd(ii)-cosd(ii +  17/20 * 1/2 * asind((c/operating_frequency)/w)));
    end
    if PLOT_SPEED
    plot(vc*[df-df1/2 df-df1/2],[-30 5], color2{count},vc*[df+df1/2 df+df1/2],[-30 5], color2{count},'linewidth',2);
    else
...
title(param_text);
print('-djpeg70','C:/Users/jeicke/Desktop/stapfigures/SINR_VERSUS_ANGLE')
return
%% test of SINR as a function of rangegate
parameterscamden;
PLOT_SPEED = true;
if PLOT_SPEED
    if(~ISBISTATIC)
...
for range_gate = ranges 
    ...
    sinr_all(:,count) = 10*log10(sinr_all(:,count));
    if PLOT_SPEED
        plot(vc*f,sinr_all(:,count),'linewidth',1)
        hold on;
    else
        plot(f,sinr_all(:,count),'linewidth',1)
    end
    count = count + 1;
end
ylabel('SINR (dB)');
...
grid on
param_text = ['SINR VERSUS RANGEGATE' sprintf('\nPRF %d HZ PULSES %d VELOCITY %d',prf,Npulses,v)];
title(param_text);
print('-djpeg70','C:/Users/jeicke/Desktop/stapfigures/SINR_VERSUS_RANGEGATE')
%% test range walking
parameterscamden;
PLOT_SPEED = true;
if PLOT_SPEED
...
range_pairs{1} = [15];
range_pairs{2} = [1 ];
range_pairs{3} = [14 15 16];
ALLOWWALK = true;
for ii = 1:length(range_pairs)
    range_gate  = range_pairs{ii}
    radar_velocity = v * [sind(angle); cosd(angle); 0];
    mimocamden;
    sinr_all(:,count) = sinr;
    f = doppler_bins';%prf/transmitters * (-Npulses/2:Npulses/2-1)/(Npulses/2);
...
legend('10.5 km range gate','0 km range gate','9.7 10.42 11.24 km range gates','location','southwest')
ylabel('SINR (dB)');
if PLOT_SPEED
    axis([vc*f(1) vc*f(end) -30 5])
    xlabel('SPEED (m/s)')
...
param_text = ['SINR VERSUS RANGEGATE' sprintf('\nPRF %d HZ PULSES %d VELOCITY %d',prf,Npulses,v)];
title(param_text);
print('-djpeg70','C:\Users\jeicke\Desktop\figures\RANGE_WALK')
%% test range folding
figure;
parameters;
v= 100;
